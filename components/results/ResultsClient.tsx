...
export function ResultsClient() {
  ...
  async function share() {
    ...
    const status = await shareText("My SAMJH Persona", `I scored ${result.socialIq} Social IQ and got ${persona.name}. Beat my score.`, url);
  ...
  function copyInstagramCaption() {
    ...
    setMessage("Instagram caption copied. Add screenshot, then start the fight.");
  ...
  async function saveImage() {
    ...
    const image = await toPng(storyCardRef.current, { pixelRatio: 2, cacheBust: true });
  ...
  return (
    <main className="mx-auto min-h-screen w-full max-w-md px-4 py-5">
      ...
      <ResultCard cardRef={storyCardRef} persona={persona} scores={result.scores} socialIq={result.socialIq} />
      ...
      <section className="mt-8">
        <p className="mb-3 text-center text-sm font-black text-cream/65">Square share layout</p>
